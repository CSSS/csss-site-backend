...
def create_watermark(
    ...
    for i in range(1, 4  * int(width), int(width/density)):
        for j in range(1, 8 * int(height), int(height/density)):
    ...
    warning_pdf.setFont("Helvetica", 100 / density)
    warning_pdf.drawString(width / density / 10, width / density / 10, f"This exam can only be viewed by {computing_id}")
