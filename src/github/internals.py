...
async def _github_request_get(url: str, token: str) -> Response | None:
    result = requests.get(
        url,
        headers={
...
async def _github_request_post(url: str, token: str, post_data: Any) -> Response | None:
...
async def _github_request_delete(url: str, token: str) -> Response | None:
...
async def _github_request_put(url: str, token: str, put_data: Any) -> Response | None:
    result = requests.put(
        url,
        headers={
            "Accept": "application/vnd.github+json",
...
async def get_user_by_username(username: str) -> GithubUser | None:
    """
...
async def get_user_by_id(uid: str) -> GithubUser | None:
    """
...
async def delete_user_from_org(username: str | None, org: str = GITHUB_ORG_NAME) -> None:
    if username is None:
        raise ValueError("Username cannot be empty")

    result = await _github_request_delete(f"https://api.github.com/orgs/{org}/memberships/{username}", GITHUB_TOKEN)

    # Logging here potentially?
    if result.status_code != 204:
        raise Exception(
            f"Status code {result.status_code} returned when attempting to delete user {username} from organization {org}"
        )


async def list_teams(org: str = GITHUB_ORG_NAME) -> list[str]:
    result = await _github_request_get(f"https://api.github.com/orgs/{org}/teams", GITHUB_TOKEN)
    return [GithubTeam(team["id"], team["url"], team["name"], team["slug"]) for team in result.json()]


async def list_team_members(team_slug: str, org: str = GITHUB_ORG_NAME):
    result = await _github_request_get(f"https://api.github.com/orgs/{org}/teams/{team_slug}/members", GITHUB_TOKEN)
    return [GithubUser(user["login"], user["id"], user["name"]) for user in result.json()]


async def add_user_to_team(username: str, team_slug: str, org: str = GITHUB_ORG_NAME) -> None:
    ...
    if result.status_code != 200:
        result_json = result.json()
        raise Exception(
            f"Status code {result.status_code} returned when attempting to add user to team: {result_json['message']}"
        )


async def remove_user_from_team(username: str, team_slug: str, org: str = GITHUB_ORG_NAME) -> None:
    ...
    if result.status_code != 204:
        raise Exception(
            f"Status code {result.status_code} returned when attempting to delete user {username} from team {team_slug}"
        )
...
async def list_members(
    ...
    result = await _github_request_get(
        f"https://api.github.com/orgs/{org}/members?per_page={page_size}&page={page_number}", GITHUB_TOKEN
    )

    if result.status_code != 200:
        raise Exception(f"Got result with status_code={result.status_code}, and contents={result.text}")

    return [(user["login"], user["id"]) for user in result.json()]
