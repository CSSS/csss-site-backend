...
async def test__get_officers(client: AsyncClient):
    ...
    response = await client.get("/api/officers/all?include_future_terms=false")
    assert response.status_code == 200
    ...
    response = await client.get("/api/officers/all?include_future_terms=true")
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test__get_officer_terms(client: AsyncClient):
    response = await client.get(f"/api/officers/terms/{load_test_db.SYSADMIN_COMPUTING_ID}?include_future_terms=false")
    assert response.status_code == 200
    assert len(response.json()) == 2
    assert response.json()[0]["nickname"] == "G2"
    assert response.json()[1]["nickname"] == "G1"

    response = await client.get("/api/officers/terms/balargho?include_future_terms=false")
    assert response.status_code == 200
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    response = await client.get(f"/api/officers/info/{load_test_db.SYSADMIN_COMPUTING_ID}")
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
async def test__update_officer_term(client: AsyncClient):
    ...
    response = await client.delete("/api/officers/term/1")
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
async def test__get_current_officers_admin(admin_client: AsyncClient):
    ...
    response = await admin_client.get("/api/officers/current")
    assert response.status_code == 200
...
async def test__admin_get_officer_term(admin_client: AsyncClient):
    response = await admin_client.get(
        f"/api/officers/terms/{load_test_db.SYSADMIN_COMPUTING_ID}?include_future_terms=false"
    )
    assert response.status_code == 200
    assert response.json() != []
    assert len(response.json()) == 2
...
async def test__admin_get_officer_term_with_future(admin_client: AsyncClient):
    response = await admin_client.get(
        f"/api/officers/terms/{load_test_db.SYSADMIN_COMPUTING_ID}?include_future_terms=true"
    )
    assert response.status_code == 200
    assert response.json() != []
...
async def test__admin_get_other_officer_term_with_future(admin_client: AsyncClient):
    response = await admin_client.get("/api/officers/terms/ehbc12?include_future_terms=true")
    assert response.status_code == 200
    assert response.json() == []


async def test__get_single_valid_officer_info(admin_client: AsyncClient):
    response = await admin_client.get("/api/officers/info/abc11")
    assert response.status_code == 200
    assert response.json() != {}
    assert response.json()["legal_name"] == "Person A"
    response = await admin_client.get(f"/api/officers/info/{load_test_db.SYSADMIN_COMPUTING_ID}")
    assert response.status_code == 200
    assert response.json() != {}
    response = await admin_client.get("/api/officers/info/balargho")
...
async def test__admin_patch_officer_term(admin_client: AsyncClient):
    ...
    response = await admin_client.get("/api/officers/terms/abc11?include_future_terms=true")
    assert response.status_code == 200
