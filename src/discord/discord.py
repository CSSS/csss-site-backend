...
async def _discord_request(url: str, token: str) -> Response:
    result = requests.get(
        url,
        headers={
...
async def get_channel(cid: str, gid: str = ACTIVE_GUILD_ID) -> Channel | None:
    ...
    if channel is None:
        ...
        return Channel(
            channel["id"], channel["type"], channel["guild_id"], channel["name"], channel["permission_overwrites"]
        )


async def get_all_channels(gid: str = ACTIVE_GUILD_ID) -> list[str]:
    url = f"https://discord.com/api/v10/guilds/{gid}/channels"
    result = await _discord_request(url, TOKEN)
...
async def get_role_name_by_id(rid: str, gid: str = ACTIVE_GUILD_ID) -> str:
    roles = await get_all_roles(gid)
    return roles[rid][0]


async def get_role_by_id(rid: str, gid: str = ACTIVE_GUILD_ID) -> dict | None:
...
async def get_user_roles(uid: str, gid: str = ACTIVE_GUILD_ID) -> list[str]:
...
async def get_all_roles(gid: str = ACTIVE_GUILD_ID) -> dict[str, list[str]]:
    """
...
async def get_guild_members_with_role(rid: str, gid: str = ACTIVE_GUILD_ID) -> list[GuildMember]:
    # base case
...
async def get_guild_members(gid: str = ACTIVE_GUILD_ID) -> list[GuildMember]:
    # base case
    url = f"https://discord.com/api/v10/guilds/{gid}/members?limit=1000"
    result = await _discord_request(url, TOKEN)
...
async def get_categories(gid: str = ACTIVE_GUILD_ID) -> list[str]:
    url = f"https://discord.com/api/v10/guilds/{gid}/channels"
    result = await _discord_request(url, TOKEN)
...
async def get_channels_by_category_name(category_name: str, gid: str = ACTIVE_GUILD_ID) -> list[Channel]:
    url = f"https://discord.com/api/v10/guilds/{gid}/channels"
    result = await _discord_request(url, TOKEN)
    ...
    category_id = next(
        ...
        if category["type"] == DISCORD_CATEGORY_ID and category["name"] == category_name
    )
    channels = [
        Channel(channel["id"], channel["type"], channel["guild_id"], channel["name"], channel["permission_overwrites"])
...
async def get_channels_by_category_id(cid: str, gid: str = ACTIVE_GUILD_ID) -> list[Channel]:
    url = f"https://discord.com/api/v10/guilds/{gid}/channels"
    result = await _discord_request(url, TOKEN)
    ...
    channels = [
        Channel(channel["id"], channel["type"], channel["guild_id"], channel["name"], channel["permission_overwrites"])
        for channel in result_json
        if channel["type"] != DISCORD_CATEGORY_ID and channel["parent_id"] == cid
    ]
    return channels


async def search_user(starts_with: str, limit: int = 1, gid: str = ACTIVE_GUILD_ID) -> list[User]:
    """
...
async def search_username(username_starts_with: str, gid: str = ACTIVE_GUILD_ID) -> list[User]:
    """
    ...
    return [user for user in user_list if user.username.startswith(username_starts_with) and user.discriminator == "0"]
