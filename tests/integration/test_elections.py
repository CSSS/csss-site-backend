...
import load_test_db
from auth.crud import create_user_session, update_site_user, get_computing_id
from database import SQLALCHEMY_TEST_DATABASE_URL, DatabaseSessionManager
from main import app
from elections.crud import (
# election crud
get_all_elections, get_election, create_election, update_election, delete_election, 
# election registration crud
get_all_registrations, get_all_registrations_in_election, add_registration, update_registration, delete_registration,

# info crud
get_nominee_info, create_nominee_info, update_nominee_info,
)
...
async def test_endpoints(client, database_setup):
    ...
    response = await client.get(f"/elections/by_name/{election_name}")
    assert response.status_code == 200
    assert response.json() != {}
    ...
    response = await client.get(f"/elections/register/{election_name}")
    assert response.status_code == 401
    
    response = await client.get(f"/elections/nominee_info")
    assert response.status_code == 401
    
    

    response = await client.post(f"/elections/by_name/{election_name}", params={
    ...
    response = await client.post(f"/elections/register/{election_name}", params={
        "position": "president",
        
    })
    assert response.status_code == 401 # unauthorized access to register candidates
    
    response = await client.patch(f"/elections/by_name/{election_name}",  params={
    ...
    response = await client.patch(f"/elections/register/{election_name}", params={
        "position": "president",
        "speech": "I would like to run for president because I'm the best in Valorant at SFU." 
    })
    assert response.status_code == 401
    
    response = await client.patch(f"/elections/nominee_info", params={
    ...
    response = await client.delete(f"/elections/by_name/{election_name}")
    assert response.status_code == 401
    
    response = await client.delete(f"/elections/register/{election_name}", params={
        "election_name": "test election 4",
        "position": "president",
    })
    assert response.status_code == 401
...
async def test_endpoints_admin(client, database_setup):
    ...
    response = await client.post(f"/elections/by_name/testElection4", params={
        "election_type": "general_election",
    ...
    response = await client.post(f"/elections/register/test election    1", params={
        "position": "president",   
    })
    ...
    response = await client.post(f"/elections/register/unknownElection12345", params={
        "position": "president",   
    })
    assert response.status_code == 400
    ...
    response = await client.patch(f"/elections/by_name/testElection4", params={
        "election_type": "general_election",
        "datetime_start_nominations": (datetime.now() - timedelta(days=1)).isoformat(),
        "datetime_start_voting": (datetime.now() + timedelta(days=7)).isoformat(),
        "datetime_end_voting": (datetime.now() + timedelta(days=14)).isoformat(),
    ...
    response = await client.patch(f"/elections/register/testElection4", params={
        "position": "president",
    ...
    # delete an election
    response = await client.delete(f"/elections/by_name/testElection4")
    assert response.status_code == 200
    ...
    # update nominee info
    response = await client.patch("/elections/nominee_info", params={
        "full_name": "Puneet N",
