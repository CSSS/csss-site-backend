...
async def test_endpoints(client, database_setup):
    ...
    response = await client.get("/elections/nominee_info")
    assert response.status_code == 401
...
async def test_endpoints_admin(client, database_setup):
    ...
    response = await client.post("/elections/by_name/testElection4", params={
        "election_type": "general_election",
    ...
    response = await client.post("/elections/register/test election    1", params={
        "position": "president",
    })
    ...
    response = await client.post("/elections/register/unknownElection12345", params={
        "position": "president",
    })
    assert response.status_code == 400
    ...
    response = await client.patch("/elections/by_name/testElection4", params={
        "election_type": "general_election",
        "datetime_start_nominations": (datetime.now() - timedelta(days=1)).isoformat(),
        "datetime_start_voting": (datetime.now() + timedelta(days=7)).isoformat(),
        "datetime_end_voting": (datetime.now() + timedelta(days=14)).isoformat(),
    ...
    response = await client.patch("/elections/register/testElection4", params={
        "position": "president",
    ...
    # delete an election
    response = await client.delete("/elections/by_name/testElection4")
    assert response.status_code == 200
